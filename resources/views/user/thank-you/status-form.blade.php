...
<form action="{{ route('update.status') }}" method="POST" class="px-8">
    ...
    <div class="md:gap-6 mb-2">
        <div class="col-span-6 sm:col-span-3">
            <label for="status" class="block text-sm font-medium text-gray-700">Status: Click the button if you already
                left the area.</label>
            <select class="hidden" id="status" name="status" autocomplete="status"
                ...
                <option selected @if ($registrationData->status === 'already_left')
                    selected
