    ...
    <div x-show="open" x-cloak>
        <div class="absolute bg-slate-50   shadow-md rounded-md top-2/4 left-2/4 w-9/12 -translate-y-2/4 -translate-x-2/4"
            ...
            <header class="flex justify-between items-center  p-4 border rounded-md border-b-1 border-gray-200">
                <h2 class="font-medium">Upload a video or image</h2>
            ...
            <form class="px-4 py-2" action="{{ route('store-feature') }}" method="POST" enctype="multipart/form-data">
                @csrf
                {{-- property image --}}
                <input name="property_id" type="hidden" value="{{ $properties->id }}">
                <div class="text-left">
                    <label for="feature" class="block mb-2 text-sm font-medium text-gray-700">
                        Feature
                    </label>
                    <input class="w-full" id="feature" name="feature" type="file" required>
                </div>
