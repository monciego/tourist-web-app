<div x-data="{open:false}" class="inline">
    ...
    <div x-show="open" x-cloak>
        <div
            class="absolute bg-slate-50 top-0 left-0 w-full -translate-y-0 -translate-x-0 shadow-md rounded-md z-[999]">
            ...
            <form class="px-4 py-2" action="{{ route('store-image-two') }}" method="POST" enctype="multipart/form-data">
                @csrf
                {{-- property image --}}
                <input name="property_id" type="hidden" value="{{ $properties->id }}">
                <div class="text-left">
                    <label for="image_two" class="block mb-2 text-sm font-medium text-gray-700">
                        Image Two
                    </label>
                    <input class="w-full" id="image_two" name="image_two" type="file" required>
                </div>
