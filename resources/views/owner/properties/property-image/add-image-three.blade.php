<div x-data="{open:false}" class="inline">
    ...
    <div x-show="open" x-cloak>
        <div
            ...
            <form class="px-4 py-2" action="{{ route('store-image-three') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                {{-- property image --}}
                <input name="property_id" type="hidden" value="{{ $properties->id }}">
                <div class="text-left">
                    <label for="image_three" class="block mb-2 text-sm font-medium text-gray-700">
                        Image Three
                    </label>
                    <input class="w-full" id="image_three" name="image_three" type="file" required>
                </div>
