<div x-data="{open:false}" class="inline">
    ...
    <div x-show="open" x-cloak>
        <div class="absolute bg-slate-50 h-2/4 shadow-md rounded-md top-2/4 left-2/4 w-full -translate-y-2/4 -translate-x-2/4"
            ...
            <form class="px-4 py-2" action="{{ route('store-image-four') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                {{-- property image --}}
                <input name="property_id" type="hidden" value="{{ $properties->id }}">
                <div class="text-left">
                    <label for="image_four" class="block mb-2 text-sm font-medium text-gray-700">
                        Image Four
                    </label>
                    <input class="w-full" id="image_four" name="image_four" type="file" required>
                </div>
